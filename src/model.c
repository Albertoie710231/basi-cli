...
GenerateResult generate(
    ...
    size_t prompt_len)
{
    /* In-process decode removed — all generation goes through llama-server. The
       ctx/vocab/smpl/prompt_len params are retained for the callers' signatures. */
    (void) ctx; (void) vocab; (void) smpl; (void) prompt_len;
    return generate_server(prompt);
}
