...
#include "backend.h"   /* backend_active — which llama-server binary to run */
...
int srvgen_write_launch_script(const SrvLaunch *cfg, const char *path) {
    ...
    /* Which declared llama-server binary this script was composed for. BASI reuses
     * the script only when BOTH markers still match, so switching backend on an
     * unchanged model regenerates instead of silently re-running the old binary. */
    if (cfg->backend_name && *cfg->backend_name)
        fprintf(f, "# BASI-BACKEND: %s\n", cfg->backend_name);
    fprintf(f, "\n");
    fprintf(f, "exec \"%s\" \\\n", bin);
    fprintf(f, "  -m \"%s\" \\\n", model);
    fprintf(f, "  -ngl %d -c %d \\\n", cfg->ngl, cfg->ctx);
...
/* Read a "# <key>: value" marker from a launch script into `out`.
 * Returns 1 if the marker exists, 0 if the file or the marker is absent. */
static int read_script_marker(const char *path, const char *key,
    ...
    while (fgets(line, sizeof line, f)) {
        char *m = strstr(line, key);
        if (!m) continue;
        m += strlen(key);
        while (*m == ' ') m++;
        size_t len = strlen(m);
        while (len > 0 && (m[len-1]=='\n' || m[len-1]=='\r' || m[len-1]==' ')) len--;
        if (out && outn) { if (len >= outn) len = outn - 1; memcpy(out, m, len); out[len] = '\0'; }
        found = 1;
        break;
    }
    fclose(f);
...
int srvgen_script_matches(const char *path, const char *model_path) {
    ...
    char got[1024];
    if (!read_script_marker(path, "# BASI-MODEL:", got, sizeof got)) return 0;
    return strcmp(got, model_path) == 0;
}

int srvgen_script_backend_matches(const char *path, const char *backend_name) {
    if (!path || !backend_name || !*backend_name) return 1;
    char got[128];
    /* No marker means a hand-written script or one predating backend selection.
     * BASI's contract is that an edited script is honored, so this counts as a
     * match rather than silently overwriting the user's work. main.c warns if such
     * a script doesn't actually name the selected binary. */
    if (!read_script_marker(path, "# BASI-BACKEND:", got, sizeof got)) return 1;
    return strcmp(got, backend_name) == 0;
}
...
int srvgen_script_params(const char *path, int *ngl, int *ctx) {
    ...
    while (fgets(line, sizeof line, f)) {
        ...
        for (char *tok = strtok_r(line, " \t\r\n", &save); tok;
             tok = strtok_r(NULL, " \t\r\n", &save)) {
            int *dst = NULL;
            if (!strcmp(tok, "-ngl") || !strcmp(tok, "--gpu-layers") ||
                !strcmp(tok, "--n-gpu-layers"))                       dst = ngl;
            else if (!strcmp(tok, "-c") || !strcmp(tok, "--ctx-size")) dst = ctx;
            if (!dst) continue;
...
int srvgen_script_uses_bin(const char *path, const char *server_bin) {
    if (!path || !server_bin || !*server_bin) return 1;
    FILE *f = fopen(path, "r");
    if (!f) return 0;
    char line[1024]; int uses = 0;
    while (fgets(line, sizeof line, f)) {
        if (line[0] == '#') continue;              /* markers/doc comments */
        if (strstr(line, server_bin)) { uses = 1; break; }
    }
    fclose(f);
    return uses;
}
