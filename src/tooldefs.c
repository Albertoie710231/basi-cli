...
static const BasiToolDef TOOLS[] = {
    ...
    { "code_context", "Return clangd's structural info (signature, type, doc) for a top-level C symbol. Authoritative for C — prefer it over grep when you need a symbol's signature, type or definition site. Requires the symbol NAME, so it cannot discover names on its own: to get the list first, run 'ctags -x --c-kinds=f <file>' via bash.",
